...
void HashTable :: Delete(TMemo *Memo, int num)
{
	...
	if(!marker) ShowMessage("Íåò ýëåìåíòà ñ òàêèì êëþ÷îì");
}
...
void HashTable :: Search(TMemo *Memo, int num)
{
	...
	if(!elements[i])
	{
		ShowMessage("Íåò ýëåìåíòà ñ òàêèì êëþ÷îì");
        return;
	}
	if(elements[i]->key == num)
	{
		Memo->Lines->Add("Ýëåìåíò íàéäåí");
		Memo->Lines->Add("Êëþ÷: " + IntToStr(elements[i]->key) + "   Çíà÷åíèå: " +
		elements[i]->info);
	}
	else
	{
		...
		for(HashNode *temp = elements[i]->pNext; temp; temp = temp->pNext)
		{
			if(temp->key == num)
			{
				marker++;
                Memo->Lines->Add("Ýëåìåíò íàéäåí");
				Memo->Lines->Add("Êëþ÷: " + IntToStr(temp->key) + "   Çíà÷åíèå: " +
				temp->info);
				break;
            }
		}
...
void HashTable :: Decrease(TMemo *Memo)
{
   ...
   Memo->Lines->Clear();
   for(int i = 0; i < length; i++)
   Memo->Lines->Add("Êëþ÷: " + IntToStr(arr[i]->key) + "   Çíà÷åíèå: " + arr[i]->info);
   Memo->Lines->Add("\n");
}
